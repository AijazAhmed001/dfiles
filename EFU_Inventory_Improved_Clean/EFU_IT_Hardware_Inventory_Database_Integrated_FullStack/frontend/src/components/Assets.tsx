...
import { api, type PageMeta } from '../api'
...
const getOriginalAssetNumber = (assetCode: string) => {
  const match = assetCode.trim().toUpperCase().match(/(\d+)$/)
...
export default function Assets({ onNavigate }: AssetsProps) {
  ...
  const displayAssets = useMemo(() => {
    ...
    return sortedAssets.map(asset => {
      ...
      const nextNumber = (prefixCounters.get(prefix) || 0) + 1

      prefixCounters.set(prefix, nextNumber)

      return {
        asset,
        displayCode: `EFU-${prefix}-${String(nextNumber).padStart(4, '0')}`,
      }
    })
  }, [assets])
            ...
            >
              ...
              <tbody>
                {displayAssets.map(({ asset, displayCode }) => {
                  const badge = getStatusStyle(asset.status)
