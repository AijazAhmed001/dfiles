...
import { API_URL, ApiError, authApi, saveSession } from '../api'
...
export default function Login({ onLogin }: Props) {
  ...
  const handleForgot = async () => {
    const address = window.prompt('Enter the email address for your account:', email)
    if (!address) return
    setError('')
    try {
      const result = await authApi.forgotPassword(address)
      if (result?.resetToken) {
        const next = window.prompt('Development reset token created. Enter a new password:')
        if (next) {
          await authApi.resetPassword(result.resetToken, next)
          window.alert('Password reset successfully.')
        }
      } else {
        window.alert('If the account exists, a password-reset email will be sent.')
      }
  ...
  const handleGoogleLogin = () => {
    ...
    if (!googleAuthUrl) {
      setError('Google sign-in is not configured yet. Please use your EFU email and password.')
      return
    }
    ...
    >
        ...
        <aside className="login-hero">
          ...
          <div className="hero-content">
            <header className="brand-row">
              <img src="/efu-login-logo.png" className="brand-logo" alt="EFU logo" />
              <div className="brand-copy">
                <div className="brand-title">EFU General Insurance</div>
                <div className="brand-subtitle">IT Hardware Inventory System</div>
              </div>
            </header>
            ...
            <div className="hero-copy">
              <h1>
                IT Assets.<br />
                <span>Managed<br />Better.</span>
              </h1>
              <div className="hero-accent" />
              <p>
                A secure and intelligent workspace to track, allocate and control your hardware inventory in <strong>real-time.</strong>
              </p>
            </div>
            ...
            <div className="feature-grid" aria-label="System capabilities">
              {features.map(({ icon: Icon, title, description, tone }) => (
                <article className={`feature-card feature-card-${tone}`} key={title}>
                  ...
                  <h3>{title}</h3>
                  <p>{description}</p>
                  <ArrowRight size={20} className="feature-arrow" aria-hidden="true" />
                </article>
              ))}
            </div>

            <div className="hero-info-bar" aria-label="Platform qualities">
              <div className="hero-info-item hero-info-secure">
                <span className="hero-info-icon"><ShieldCheck size={22} /></span>
                <span><strong>Secure</strong><small>Enterprise grade security</small></span>
              </div>
              <div className="hero-info-item hero-info-reliable">
                <span className="hero-info-icon"><Zap size={22} /></span>
                <span><strong>Reliable</strong><small>99.9% system availability</small></span>
              </div>
              <div className="hero-info-item hero-info-protected">
                <span className="hero-info-icon"><LockKeyhole size={21} /></span>
                <span><strong>Protected</strong><small>Your data is always protected</small></span>
              </div>
            </div>
          </div>
        </aside>
          ...
          <div className="login-card">
            <div className="login-card-topline">
              ...
              <div className="login-card-actions">
                ...
                >
                  ...
                  <RefreshCw
                    size={13}
                    className={serverStatus === 'checking' ? 'server-refresh is-spinning' : 'server-refresh'}
                  />
                </button>
                <button
                  type="button"
            ...
            <div className="card-heading">
              <div className="card-logo-wrap">
                ...
                <img src="/efu-login-logo.png" className="card-logo" alt="EFU logo" />
              </div>
              <h2>Welcome Back</h2>
              <p>
            ...
            <div className="login-feedback" aria-live="polite" aria-atomic="true">
              {error && (
                <div id="login-feedback-message" className="login-error" role="alert">
                  {error}
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} autoComplete="off" className="login-form">
              <div className="field-group">
                ...
                <div className="input-wrap">
                  ...
                  <input
                    ...
                    aria-describedby={error ? 'login-feedback-message' : undefined}
                  />
                </div>
              </div>
              ...
              <div className="form-options">
                ...
                <button type="button" className="text-link" onClick={handleForgot}>
                  Forgot Password?
                </button>
              </div>

              <button type="submit" className="primary-login" disabled={loading}>
                {loading
                ...
                {!loading && <ArrowRight size={20} className="login-button-arrow" aria-hidden="true" />}
              </button>
      ...
      <style>{`
        ...
        .hero-info-bar {
          ...
          box-shadow: 0 12px 32px rgba(0, 13, 38, 0.2);
          backdrop-filter: blur(14px);
        }
        ...
        .hero-info-secure .hero-info-icon,
        .hero-info-secure strong { color: #35e6a1; }

        .hero-info-reliable .hero-info-icon,
        .hero-info-reliable strong { color: #45a7ff; }

        .hero-info-protected .hero-info-icon,
        .hero-info-protected strong { color: #a779ff; }
        ...
        .theme-light .hero-copy h1 {
          color: #0b1d36;
          text-shadow: 0 2px 15px rgba(255, 255, 255, 0.68);
        }

        .theme-light .brand-subtitle {
          color: rgba(11, 29, 54, 0.68);
        }

        .theme-light .hero-copy p {
          color: rgba(11, 29, 54, 0.76);
          text-shadow: 0 1px 9px rgba(255, 255, 255, 0.72);
        }

        .theme-light .feature-card {
          border-color: rgba(65, 108, 140, 0.16);
          background: rgba(255, 255, 255, 0.88);
          box-shadow:
            inset 0 1px 0 rgba(255, 255, 255, 0.95),
            0 12px 30px rgba(32, 83, 117, 0.14);
        }
        ...
        .theme-light .hero-info-bar {
          border-color: rgba(65, 108, 140, 0.14);
          background: rgba(255, 255, 255, 0.9);
          box-shadow: 0 12px 30px rgba(32, 83, 117, 0.13);
        }

        .theme-light .hero-info-icon {
          background: rgba(16, 92, 145, 0.07);
        }

        .theme-light .hero-info-item small {
        ...
        .theme-dark .secure-pill {
          color: #cfeaff;
          border-color: rgba(95, 160, 222, 0.38);
          background: rgba(31, 86, 150, 0.25);
        }
