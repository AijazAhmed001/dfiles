...
import { downloadCompleteReportPdf, type CompleteReportExport } from '../utils/reportPdf'
import {
  useCallback,
  useEffect,
  useMemo,
...
function SearchField({
  ...
  return (
    <div className="reports-search">
      <Search size={17} className="reports-search__icon" />
...
function EmptyState({
  ...
  return (
    <div className="reports-empty">
      <div className="reports-empty__icon">{icon}</div>
...
export default function Reports() {
  ...
  const [historyAssetSearch, setHistoryAssetSearch] = useState('')
  ...
  const loadInventory = useCallback(async () => {
    try {
      ...
      const response = await api.get<Asset[]>('/reports/inventory')
  ...
  const loadAudits = useCallback(async () => {
    try {
      ...
      const response = await api.getPage<AuditItem>(
        '/reports/audit?page=1&limit=500',
      )
  ...
  const loadAssetHistory = useCallback(async (assetId: string) => {
    if (!assetId) {
      setHistory(null)
      return
    }
  ...
  }, [tab, auditLoaded, loadAudits])

  useEffect(() => {
    void loadAssetHistory(selectedAssetId)
  }, [selectedAssetId, loadAssetHistory])
  ...
  const historyAssetOptions = useMemo(() => {
    const query = historyAssetSearch.trim().toLowerCase()
    return assets.filter(asset => !query || [
      asset.model,
      asset.assetCode,
      asset.serialNumber,
      asset.assetType?.name,
    ].filter(Boolean).join(' ').toLowerCase().includes(query))
  }, [assets, historyAssetSearch])
  ...
  const inventorySummary = useMemo(() => {
    const totalValue = assets.reduce(
      (total, asset) => total + (asset.purchaseCost || 0),
      0,
    )

    const inStock = assets.filter(asset =>
      ['IN_STOCK', 'AVAILABLE'].includes(
    ...
    const retired = assets.filter(
      asset => asset.status?.toUpperCase() === 'RETIRED',
    ).length
  ...
  const sortedHistory = useMemo(() => {
    ...
    const allocations = Array.isArray(history.allocations) ? history.allocations : []
    const storedStatusHistory = Array.isArray(history.statusHistory) ? history.statusHistory : []
    const hasCreatedEvent = storedStatusHistory.some(item => item.eventType?.toUpperCase() === 'CREATED')
    const createdAt = history.asset.createdAt || history.asset.purchaseDate
    const statusHistory: StatusHistoryItem[] = !hasCreatedEvent && createdAt
      ? [...storedStatusHistory, {
    ...
    const allocationEvents: StatusHistoryItem[] = allocations.flatMap(allocation => {
      const legacyEmployee = allocation.employee && typeof allocation.employee === 'object'
        ? allocation.employee as Record<string, unknown>
        : undefined
      const employeeName = textValue(allocation.employeeName) || textValue(legacyEmployee)
      const employeeCode = textValue(allocation.employeeCode) ||
        (typeof legacyEmployee?.employeeId === 'string' ? legacyEmployee.employeeId : undefined)
      const department = textValue(allocation.department) || textValue(legacyEmployee?.department)
      const location = textValue(allocation.location)
      const allocationStatus = statusHistory.find(item =>
        item.eventType === 'ALLOCATED' &&
        Math.abs(new Date(item.effectiveAt).getTime() - new Date(allocation.allocationDate).getTime()) < 60000,
      )
      const allocated: StatusHistoryItem = {
        id: `allocation-${allocation.id}`,
        fromStatus: 'IN_STOCK',
        toStatus: 'ALLOCATED',
        eventType: 'ALLOCATED',
        effectiveAt: allocation.allocationDate,
        remarks: allocation.remarks,
        employeeName,
        employeeCode,
        department,
        location,
        returnedAt: allocation.returnedAt || undefined,
        performedBy: allocationStatus?.performedBy,
      }
      if (!allocation.returnedAt) return [allocated]
      const returnStatus = statusHistory.find(item =>
        ['RETURNED', 'REVOKED'].includes(item.eventType) &&
        Math.abs(new Date(item.effectiveAt).getTime() - new Date(allocation.returnedAt!).getTime()) < 60000,
      )
      return [allocated, {
        ...allocated,
        ...
        toStatus: 'IN_STOCK',
        eventType: 'RETURNED',
        effectiveAt: allocation.returnedAt,
        performedBy: returnStatus?.performedBy,
      }]
    })

    const detailedTypes = new Set(allocationEvents.map(item => item.eventType))
    const statusEvents = statusHistory.filter(item => !detailedTypes.has(item.eventType))

    return [...statusEvents, ...allocationEvents].sort(
      (first, second) =>
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const report = await api.post<CompleteReportExport>('/reports/complete-export', {})
      await downloadCompleteReportPdf(report)
    } catch (requestError) {
  ...
  return (
    <section className="reports-page">
      <header className="reports-header">
        <div>
          <p className="reports-header__eyebrow">Analytics and compliance</p>

          <h1>Reports</h1>

          <p className="reports-header__description">
            Review live inventory, asset lifecycle history and system
            audit activity.
          </p>
        </div>
        ...
        <div className="reports-header__actions">
          ...
          >
            {isPdfDownloading ? <LoaderCircle size={17} className="reports-spin" /> : <Download size={17} />}
            {isPdfDownloading ? 'Preparing PDF...' : 'Download Complete PDF'}
          </button>
          ...
          >
            <RefreshCw
              size={17}
              className={currentLoading ? 'reports-spin' : ''}
            />
            Refresh
          </button>
        </div>
        ...
        <>
          <div className="reports-summary">
            ...
            <article className="reports-summary-card">
              ...
              <div>
                <span>Allocated</span>
                <strong>{inventorySummary.allocated}</strong>
              </div>
            </article>
          ...
          <div className="reports-panel">
            ...
            {isInventoryLoading && !inventoryLoaded ? (
              <div className="reports-loading">
                <LoaderCircle size={30} className="reports-spin" />
                        ...
                        <tr key={asset.id}>
                          <td>
                            <div className="reports-asset-cell">
                              ...
                              <div>
                                <strong>{asset.model || 'Unnamed asset'}</strong>
                                <span>{formatAssetCode(asset.assetCode)}</span>
                              </div>
                            </div>
                          </td>
          ...
          <aside className="reports-history-selector">
            <div className="reports-history-selector__header">
              ...
              <div>
                ...
                <p>Choose an asset to view its lifecycle.</p>
              </div>
            </div>

            <label htmlFor="asset-history-search">1. Search and choose an asset</label>
            <div className="reports-asset-picker-search">
              <Search size={16} />
              <input
                id="asset-history-search"
                value={historyAssetSearch}
                onChange={event => setHistoryAssetSearch(event.target.value)}
                placeholder="Search model, asset ID or serial number"
              />
            </div>

            <div className="reports-asset-picker" role="listbox" aria-label="Available assets">
              {historyAssetOptions.length === 0 ? (
                <div className="reports-asset-picker__empty">No assets match your search.</div>
                ...
                return (
                  ...
                  >
                    <span className="reports-asset-picker__icon"><Laptop size={16} /></span>
                    <span className="reports-asset-picker__text">
                      <strong>{asset.model || 'Unnamed asset'}</strong>
                      <small>{formatAssetCode(asset.assetCode)} · {formatStatus(asset.status)}</small>
                    </span>
            ...
            <div className="reports-history-selector__stats">
              ...
              <span>
                <Archive size={16} />
                {inventorySummary.retired} retired
              </span>
            </div>
          </aside>
              ...
              <>
                <div className="reports-history-asset">
                  <div className="reports-history-asset__main">
                    ...
                    <div>
                      <p>Asset lifecycle</p>
                      <h2>{history.asset.model}</h2>

                      <div className="reports-history-identifiers">
                        <span><b>Asset ID</b>{formatAssetCode(history.asset.assetCode)}</span>
                        <span><b>Serial Number</b>{history.asset.serialNumber || '—'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    {formatStatus(history.asset.status)}
                  </span>
                </div>
                ...
                <div className="reports-history-meta">
                  ...
                  <div>
                    ...
                    <strong>
                      {formatCurrency(history.asset.purchaseCost)}
                    </strong>
                  </div>
                      ...
                      >
                        ...
                        <div className="reports-timeline-item__content">
                          ...
                          <div className="reports-status-change">
                            <span>
                              {formatStatus(item.fromStatus || 'NEW')}
                            </span>
                            ...
                            <span>{formatStatus(item.toStatus)}</span>
                          </div>
                          ...
                          {(item.employeeName || item.department || item.location || item.performedBy) && (
                            <div className="reports-history-details">
                              {item.employeeName && <span><b>Employee</b>{item.employeeName}{item.employeeCode ? ` (${item.employeeCode})` : ''}</span>}
                              {item.department && <span><b>Department</b>{item.department}</span>}
                              {item.location && <span><b>Location</b>{item.location}</span>}
                              {item.performedBy && <span><b>Performed by</b>{item.performedBy}</span>}
                            </div>
                          )}
                        </div>
                      </article>
                      ...
                      <tr key={item.id}>
                        ...
                        <td>
                          <span
                            ...
                            title={item.entityId || 'No entity ID'}
                          >
                            {item.entityId || '—'}
                          </span>
                        </td>
