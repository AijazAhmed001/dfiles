...
public static class DbInitializer
{
    public static async Task InitializeAsync(IServiceProvider sp)
    {
        ...
        // 2. Permissions
        // -----------------------------
        var permissionDefinitions = Permissions.Catalog.Select(item => (item.Code, item.Name)).ToArray();
        ...
        // IT administrators receive no sensitive capability from their role alone.
        // Their effective permissions are explicit UserPermissions granted by a Super Admin.
        var obsoleteItAdminGrants = await db.RolePermissions
            .Where(x => x.RoleId == itAdminRole.Id).ToListAsync();
        db.RolePermissions.RemoveRange(obsoleteItAdminGrants);
