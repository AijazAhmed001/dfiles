...
public class AdminController(AppDbContext db, IConfiguration config, TokenService tokenService) : ControllerBase
{
    ...
    public IActionResult Login(LoginRequest request)
    {
        ...
        var valid = !string.IsNullOrWhiteSpace(username) && !string.IsNullOrWhiteSpace(password) &&
                    CryptographicOperations.FixedTimeEquals(Encoding.UTF8.GetBytes(request.Username), Encoding.UTF8.GetBytes(username)) &&
                    CryptographicOperations.FixedTimeEquals(Encoding.UTF8.GetBytes(request.Password), Encoding.UTF8.GetBytes(password));
