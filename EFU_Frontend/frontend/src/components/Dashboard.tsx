...
interface DashboardData {
  ...
  recentAllocations: {
    ...
    asset?: { model: string; status: string }
...
export default function Dashboard({
  ...
  const groupedDistribution = remainingDistribution.length
    ? [
        ...
        {
          name: "Other",
          value: remainingDistribution.reduce((total, item) => total + item.value, 0),
        },
      ]
    : primaryDistribution
  const assetDistribution = groupedDistribution.map((item, index) => ({
    ...item,
    color: item.name === "Other" ? "#64748B" : palette[index % 6],
  }))
    ...
    >
      ...
      >
        <div>
          ...
          >
            Good morning, System Administrator. Here is your latest inventory
            overview.
          </p>
      ...
      >
        ...
        <SectionCard title="Asset Distribution">
          ...
          {remainingDistribution.length > 0 && (
            <p style={{ margin: "10px 0 0", color: "var(--text-secondary)", fontSize: 10, textAlign: "center" }}>
              Showing the 6 largest asset types; {remainingDistribution.length} smaller type{remainingDistribution.length === 1 ? "" : "s"} combined as Other.
            </p>
          )}
        </SectionCard>
      </div>
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            ].filter((action) => action.screen !== "new-asset").map((a) => (
              <button
                key={a.label}
        ...
        >
          <div style={{ width: "100%", overflowX: "auto" }}>
            ...
            >
              <thead>
                <tr>
                  {["Asset", "Type", "Vendor", "Cost (PKR)", "Date"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
