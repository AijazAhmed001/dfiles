param([string]$BaseUrl = 'http://localhost:5002/api')
...
$badLogin = Invoke-Test 'Invalid login' POST '/auth/login' @{email='admin@efu.com.pk';password='wrong'} @(401)
$invalidLogin = Invoke-Test 'Login validation' POST '/auth/login' @{email='not-an-email';password=''} @(400)
$login = Invoke-Test 'Admin login' POST '/auth/login' @{email='admin@efu.com.pk';password='Password@123'} @(200)
...
$corsHeaders = @{ Origin='http://localhost:5173'; 'Access-Control-Request-Method'='POST'; 'Access-Control-Request-Headers'='content-type,authorization' }
$cors = Invoke-WebRequest -Uri "$BaseUrl/auth/login" -Method OPTIONS -Headers $corsHeaders -UseBasicParsing
$corsPassed = $cors.Headers['Access-Control-Allow-Origin'] -contains 'http://localhost:5173'
