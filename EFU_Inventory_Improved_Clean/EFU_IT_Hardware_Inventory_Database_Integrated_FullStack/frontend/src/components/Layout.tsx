...
import {
  LayoutDashboard, ChevronDown, ChevronRight, Monitor, Cpu, MemoryStick,
  HardDrive, Server, Building2, MapPin, Map, Users, Briefcase, Building,
  ClipboardList, PackagePlus, PackageCheck, PackageX, Clock, BarChart2,
  Settings, LogOut, Bell, Search, Moon, Sun, UserCircle, PanelLeftClose, PanelLeftOpen,
  Shield, FileText, Layers
} from 'lucide-react'
...
type Screen =
  ...
  | 'asset-history' | 'settings' | 'profile' | 'notifications' | 'asset-details'
...
const navItems: NavItem[] = [
  ...
  {
    label: 'Master Setup', icon: <Layers size={18} />,
    children: [
      { label: 'Asset Type', screen: 'asset-type', icon: <Monitor size={16} /> },
      ...
      { label: 'Operating System', screen: 'operating-system', icon: <Server size={16} /> },
      ...
      { label: 'Department', screen: 'department', icon: <Briefcase size={16} /> },
      { label: 'Office', screen: 'office', icon: <Building size={16} /> },
      { label: 'Employee', screen: 'employee', icon: <Users size={16} /> },
      { label: 'Lifecycle Policy', screen: 'lifecycle-policy', icon: <ClipboardList size={16} /> },
    ],
  },
  {
    label: 'Transactions', icon: <FileText size={18} />,
    children: [
      { label: 'New Asset', screen: 'new-asset', icon: <PackagePlus size={16} /> },
      { label: 'Asset Allocation', screen: 'asset-allocation', icon: <PackageCheck size={16} /> },
      { label: 'Asset Revocation', screen: 'asset-revocation', icon: <PackageX size={16} /> },
      { label: 'Asset Expiration', screen: 'asset-expiration', icon: <Clock size={16} /> },
    ],
  },
  {
    label: 'Reports', icon: <BarChart2 size={18} />,
    children: [
      { label: 'Asset History', screen: 'asset-history', icon: <BarChart2 size={16} /> },
    ],
  },
  { label: 'Settings', screen: 'settings', icon: <Settings size={18} /> },
]

export default function Layout({ currentScreen, onNavigate, onLogout, children }: Props) {
  const user = loadSession(false)?.user
  const initials = user?.name?.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase() || 'U'
  const [unreadCount, setUnreadCount] = useState(0)
  useEffect(() => { api.get<{ readAt?: string | null }[]>('/notifications').then(items => setUnreadCount(items.filter(item => !item.readAt).length)).catch(() => undefined) }, [currentScreen])
  ...
  const [darkMode, setDarkMode] = useState(() => {
    const savedTheme = localStorage.getItem('efu.appTheme') || localStorage.getItem('efu.loginTheme')
  ...
  return (
    <div
      ...
      style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--app-bg)' }}
    >
      {/* Sidebar */}
      <aside
        style={{
          ...
          background: 'linear-gradient(180deg, #06345f 0%, #074b83 48%, #0a5f9f 100%)',
          display: 'flex',
      ...
      >
        ...
        >
          <button
            type="button"
            title={!sidebarOpen ? 'Open sidebar' : 'EFU General IT Inventory System'}
            aria-label={!sidebarOpen ? 'Open sidebar' : 'EFU General IT Inventory System'}
            onClick={() => {
              if (!sidebarOpen) {
                setSidebarOpen(true)
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: active ? 'inset 3px 0 0 rgba(255,255,255,0.9), 0 4px 12px rgba(0,0,0,0.08)' : 'none',
                  }}
                  onMouseEnter={e => {
                    if (!active) {
                      e.currentTarget.style.background = 'rgba(255,255,255,0.08)'
                      e.currentTarget.style.color = '#fff'
                      e.currentTarget.style.transform = sidebarOpen ? 'translateX(2px)' : 'none'
                    }
                  }}
                  onMouseLeave={e => {
                    if (!active) {
              ...
              <div key={item.label} style={{ marginBottom: 5 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    border: '1px solid transparent',
                    background: groupActive ? 'rgba(255,255,255,0.10)' : 'transparent',
                    color: groupActive ? '#fff' : 'rgba(255,255,255,0.72)',
                    cursor: 'pointer',
                    fontSize: 14,
                    fontWeight: groupActive ? 650 : 450,
                    textAlign: 'left',
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 9,
                            border: 'none',
                            background: childActive ? 'rgba(255,255,255,0.16)' : 'transparent',
                            color: childActive ? '#fff' : 'rgba(255,255,255,0.68)',
                            cursor: 'pointer',
                            fontSize: 13,
        ...
        >
          <button
            ...
            style={{
              ...
              border: sidebarOpen ? '1px solid rgba(255,255,255,0.10)' : '1px solid transparent',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{
            ...
            <input
              ...
              style={{
                ...
                fontSize: 13, color: 'var(--text-primary)', background: 'var(--surface-soft)',
                outline: 'none',
              }}
            />
          </div>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', color: 'var(--theme-button-color)',
            ...
            <div ref={notificationMenuRef} style={{ position: 'relative' }}>
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', padding: 8, borderRadius: 8, position: 'relative' }}
              >
                <Bell size={18} />
                {unreadCount > 0 && <span style={{
                  ...
                  color: '#fff', fontSize: 9, lineHeight: '12px', fontWeight: 700,
                }}>{unreadCount > 9 ? '9+' : unreadCount}</span>}
              </button>
              {notifOpen && (
                <div style={{
                  position: 'absolute', right: 0, top: '110%',
                  width: 320, background: 'var(--surface-raised)', borderRadius: 12,
                  boxShadow: 'var(--dropdown-shadow)',
                  border: '1px solid var(--border)', zIndex: 100, overflow: 'hidden',
                }}>
                  <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontWeight: 600, fontSize: 14 }}>Notifications</span>
                    <button onClick={() => api.patch('/notifications/read-all', {}).then(() => setUnreadCount(0))} style={{ fontSize: 12, color: '#005BAC', cursor: 'pointer', border: 0, background: 'none' }}>Mark all read</button>
                  </div>
                  <div style={{ padding: '18px 16px', fontSize: 13, color: 'var(--text-secondary)' }}>{unreadCount ? `${unreadCount} unread notification${unreadCount === 1 ? '' : 's'}` : 'You are all caught up.'}</div>
                  <div style={{ padding: '10px 16px', textAlign: 'center' }}>
                    <button
                      onClick={() => { onNavigate('notifications'); setNotifOpen(false) }}
                      style={{ fontSize: 13, color: '#005BAC', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500 }}
                    >
            ...
            <div ref={profileMenuRef} style={{ position: 'relative' }}>
              ...
              >
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.2 }}>{user?.role?.replaceAll('_', ' ')}</div>
                </div>
              ...
              {profileOpen && (
                <div style={{
                  position: 'absolute', right: 0, top: '110%',
                  width: 200, background: 'var(--surface-raised)', borderRadius: 10,
                  boxShadow: 'var(--dropdown-shadow)',
                  border: '1px solid var(--border)', zIndex: 100, overflow: 'hidden',
                }}>
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 13, color: 'var(--text-primary)', borderBottom: '1px solid var(--border-soft)',
                      }}
                    >
                      {item.label}
                    </button>
        ...
        {/* Page content */}
        <main className="app-content" style={{ flex: 1, overflowY: 'auto', padding: '24px', background: 'var(--app-bg)' }}>
          {children}
        </main>
      </div>
    </div>
