export const API_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5003/api').replace(/\/$/, '')
...
function decodeError<T>(response: Response, payload: ApiEnvelope<T> | null) {
  const validation = payload?.errors
    ...
    : ''
  return new ApiError(
    validation || payload?.message || `Request failed with status ${response.status}`,
    response.status,
    payload?.errors,
  )
}
