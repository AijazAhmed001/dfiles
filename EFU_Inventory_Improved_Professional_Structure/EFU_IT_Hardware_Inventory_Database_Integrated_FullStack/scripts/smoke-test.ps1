param(
    [string]$BaseUrl = 'http://localhost:5003/api',
...
$corsHeaders = @{ Origin='http://localhost:5174'; 'Access-Control-Request-Method'='POST'; 'Access-Control-Request-Headers'='content-type,authorization' }
$cors = Invoke-WebRequest -Uri "$BaseUrl/auth/login" -Method OPTIONS -Headers $corsHeaders -UseBasicParsing
$corsPassed = $cors.Headers['Access-Control-Allow-Origin'] -contains 'http://localhost:5174'
