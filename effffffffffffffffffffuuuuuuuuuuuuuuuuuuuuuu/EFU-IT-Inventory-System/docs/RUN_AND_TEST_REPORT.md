...
- Aligned the default frontend/backend development ports to `5175` and `5002`.
...
- Another unrelated backend installation can use port 5000. The project defaults are now consistently `5002` for the backend and `5175` for the frontend.
...
dotnet restore
dotnet run --no-restore -- --urls http://localhost:5002 --FrontendUrl http://localhost:5175
```
