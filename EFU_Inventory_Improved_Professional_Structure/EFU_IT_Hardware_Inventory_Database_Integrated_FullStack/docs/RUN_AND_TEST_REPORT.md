...
- Aligned the default frontend/backend development ports to `5174` and `5003`.
...
- Another unrelated backend installation can use port 5000. The project defaults are now consistently `5003` for the backend and `5174` for the frontend.
...
dotnet restore
dotnet run --no-restore -- --urls http://localhost:5003 --FrontendUrl http://localhost:5174
```

Startup automatically connects to LocalDB and applies/verifies the SQL migrations.
Swagger is available at `http://localhost:5003/swagger`.
...
With the backend running on port 5003:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\smoke-test.ps1 -BaseUrl http://localhost:5003/api
```
